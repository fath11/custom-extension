  ...
  class SPrecording {
    ...
    async saveRecording(args) {
      if (this.recording) {
        ...
        if (target === "Stage") target = this.runtime.getTargetForStage().id;
        else target = this.runtime.getSpriteTargetByName(target).id;
        if (!target) return;
        fetch(await this.convertBlobToBase64(this.recording, "mp3"))
    ...
    startRecording() {
      if (!this.isRecording) {
        this.runtime.startHats("SPrecording_whenMic", { ON_OFF: "on" });
    ...
    stopRecording() {
      if (this.isRecording) {
        ...
        this.isRecording = false;
        this.runtime.startHats("SPrecording_whenMic", { ON_OFF: "off" });
      }
    }
  }

  if (Scratch.vm?.runtime) {
    Scratch.extensions.register(new SPrecording(Scratch.vm.runtime))
  } else {
    Reflect.set(window, 'tempExt', {
      Extension: SPrecording,
      info: {
        name: 'SPrecording.name',
        description: 'SPrecording.desc',
        extensionId: 'SPrecording',
        featured: true,
        disabled: false,
        iconURL: "https://sharkpool-sp.github.io/SharkPools-Extensions/extension-thumbs/Recording.svg", // The cover of your extension.
        insetIconURL: blockIconURI,
        collaborator: 'Sharkpool@TW',
        collaboratorURL: 'https://github.com/SharkPool-SP',
      },
      l10n: {
        'zh-cn': {
          'SPrecording.name': 'Recording',
          'SPrecording.desc': 'Record your voice while you run your projects!'
        },
        en: {
          'SPrecording.name': 'Recording',
          'SPrecording.desc': 'Record your voice while you run your projects!'
